    ...
    try {
      ...
      for (String path : collectOverdueTasks(session)) {
        ...
        try {
          ...
          log.error("Could not read information from task [{}], {}", path, ex);
          continue;
        }
        ...
        try {
          ...
          log.error("Could not get assignees e-mails of task [{}], {}", path, ex);
        }
      }
      ...
      log.error("Overdue tasks notifier error", ex);
    }
