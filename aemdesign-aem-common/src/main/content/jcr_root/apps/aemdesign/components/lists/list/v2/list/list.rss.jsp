...
<rss version="2.0">
  <channel>
    ...
    <title><%= escapeBody(title) %>
    </title>

    <c:if test="<%= !isEmpty(subTitle) %>">
      <description><%= escapeBody(subTitle) %>
      </description>
    </c:if>
    <ttl>1800</ttl>
    ...
    <item>
      ...
      <link><%= mappedUrl(_resourceResolver, listPage.getPath()).concat(DEFAULT_EXTENTION) %>
      </link>
      <guid><%= hashMd5(listPage.getPath()) %>
      </guid>
      ...
      <c:if test="<%= listPageImage.hasContent() %>">
        <image><%= mappedUrl(_resourceResolver, listPageImage.getSrc()) %>
        </image>
      </c:if>
    </item>
